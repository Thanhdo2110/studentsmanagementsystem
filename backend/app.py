...
def log_action(action, detail="", username="system"):
    db.session.add(ActivityLog(action=action, detail=detail, username=username))
    db.session.commit()
...
def login():
    ...
    log_action("login", f"Đăng nhập: {user.username}", user.username)
...
def register():
    ...
    log_action("register", full_name, username)
    token = create_token(user)
...
def create_student():
    ...
    if Student.query.filter_by(student_code=data["student_code"]).first(): return jsonify({"error": "Mã HS đã tồn tại"}), 400
    if Student.query.filter_by(email=data["email"]).first(): return jsonify({"error": "Email đã tồn tại"}), 400
    ...
    log_action("create_student", s.full_name, request.current_user.username)
...
def update_student(sid):
    ...
    db.session.commit()
    log_action("update_student", s.full_name, request.current_user.username)
...
def delete_student(sid):
    ...
    log_action("delete_student", name, request.current_user.username)
...
def export_students():
    ...
    log_action("export_csv", f"{len(students)} học sinh", request.current_user.username)
...
def import_students():
    ...
    for i, row in enumerate(reader, 2):
        ...
        if Student.query.filter((Student.student_code==code)|(Student.email==email)).first(): err.append(f"Dòng {i}: trùng mã/email"); continue
        db.session.add(Student(student_code=code, full_name=name, email=email, phone=row.get("phone"), gender=row.get("gender"),
            class_name=row.get("class_name"), address=row.get("address"), status=row.get("status","active")))
        ok += 1
    db.session.commit()
    log_action("import_csv", f"{ok} học sinh", request.current_user.username)
...
@token_required
def list_classrooms():
    return jsonify([c.to_dict() for c in Classroom.query.order_by(Classroom.id.desc()).all()])
...
def create_classroom():
    ...
    if Classroom.query.filter_by(class_code=data["class_code"]).first(): return jsonify({"error": "Mã lớp đã tồn tại"}), 400
    c = Classroom(class_code=data["class_code"], class_name=data["class_name"], grade=data.get("grade"),
        teacher_name=data.get("teacher_name"), capacity=data.get("capacity", 40))
    db.session.add(c); db.session.commit()
    log_action("create_class", c.class_name, request.current_user.username)
...
def update_classroom(cid):
    ...
    db.session.commit()
    log_action("update_class", c.class_name, request.current_user.username)
...
def delete_classroom(cid):
    ...
    log_action("delete_class", name, request.current_user.username)
    return jsonify({"message": "Deleted"})

@app.route("/api/classes")
@token_required
def classes():
    rows = Classroom.query.with_entities(Classroom.class_name).all()
    extra = db.session.query(Student.class_name).filter(Student.class_name.isnot(None), Student.class_name!="").distinct().all()
...
def stats():
    ...
    inactive = Student.query.filter_by(status="inactive").count()
    classes_data = db.session.query(Student.class_name, func.count(Student.id)).filter(
        Student.class_name.isnot(None), Student.class_name!="").group_by(Student.class_name).all()
